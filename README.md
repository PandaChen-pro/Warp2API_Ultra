...
├── openai_compat.py         # 🤖 OpenAI 兼容API服务 (端口: 7777)
│
...
- `PROXY_URL`: 代理地址，V2rayN 10880，Clash 7890
...
希望一次性注册指定数量并在完成后退出，可使用批量模式：

```bash
# 使用 uv（推荐）
uv run main.py register 20    # 精确注册 20 个账号

# 或使用 python 直接运行
python main.py register 20
```

说明：
- 并发度由 `config.py` 中的 `MAX_CONCURRENT_REGISTER` 控制，适当提高可加速批量注册。
- 未指定数字时（如 `python main.py register`），以“监控模式”运行：按 `TARGET_ACCOUNTS` 目标值维持账号池。
- 开始前请确认 `TEMP_MAIL_BASE_URL`、`PROXY_URL`、`FIREBASE_API_KEYS` 已在 `config.py` 正确配置。

## 📝 API 使用

服务启动后，你可以通过两个主要的API端点与系统交互。

### 1. OpenAI 兼容 API (`http://127.0.0.1:7777`)
