...
def run_warp_register():
    ...
    try:
        ...
        # 支持: python main.py register <count>
        count: int | None = None
        try:
        ...
        if count is not None:
            logger.info(f"批量注册模式: 目标 {count} 个账号")
            asyncio.run(module.main(count))
        else:
            asyncio.run(module.main())
    except Exception as e:
...
def print_usage():
    ...
    print("  python main.py register 20   # 直接注册20个账号后退出")
    print("=" * 60)
